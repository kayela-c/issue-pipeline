...
- **Follow-up (2026-09-16): LM Studio in Settings** (decision 24). LM Studio is now a sixth provider choice with a server URL, an optional API token (sealed like any key), the two models, and the context length the model is loaded with (`user_ai_providers.base_url` and `context_tokens`, migration `0007_lmstudio_settings`). Unset fields fall back to `LMSTUDIO_*` env vars, then `http://localhost:1234` and 8192. It is offered, listed, and usable only while the server runs locally (`CONTEXT` unset or `dev`, `localProvidersAvailable()` in `src/llm/index.ts`); deployed, the settings API hides it and 404s its routes, and a run with it saved fails with a message to choose another provider.
...
24. **LM Studio in Settings (2026-09-16):** while testing Phase 9, Kayela's hosted AI key did not work and she wanted to use LM Studio instead, with its API token, but it could only be set through `.env`. LM Studio is now a Settings provider like the others. It stays local-only: deployed functions cannot reach a model on someone's machine, and fetching a user-supplied URL from production would be a server-side request forgery risk, so the server offers it only when it runs locally.
