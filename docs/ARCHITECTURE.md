...
1. **One Netlify site** built from `prod` (the site's configured production branch; `dev` is the ongoing development branch -- git's default branch was renamed from `main` to `dev` on 2026-09-14): build command compiles `packages/shared` and `apps/web`; `publish = apps/web/dist`; `functions = apps/api/netlify/functions`. No base directory. `netlify.toml` at the repo root already has this -- **done**. `prod` was created off the git branch formerly called `main` (now `dev`) and pushed to `origin/prod` on 2026-09-14; how it stays in sync with `dev` going forward (merge/fast-forward on every change, or a deliberate promotion step) has not been decided.
2. **Environments:** production uses the Neon project's `main` branch (its default/primary database branch); local dev (`netlify dev`) uses the Neon `dev` branch. **These are Neon database branches, not the git branches in item 1** -- confusingly, since the 2026-09-14 rename, git also has a branch called `dev` (git's `dev` is the code counterpart of git's `prod`, unrelated to which *database* branch either one talks to; today both `netlify dev` and the deployed `prod` site would use whichever `DATABASE_URL` is configured for them, independent of the git branch name). Migrations are applied with `pnpm db:migrate` before a deploy that needs them. **Done (2026-09-14):** the Neon `main` branch had zero tables (only the Neon `dev` branch had ever been migrated); `pnpm db:migrate` was run against it and all 8 tables plus `drizzle.__drizzle_migrations` now exist.
...
20. **Production deploys build from git branch `prod` (2026-09-14):** Kayela set the Netlify site's production branch to `prod` and created it (off the git branch then called `main`) and pushed it to `origin/prod`. How `prod` is kept current -- merged/fast-forwarded on every change, or advanced only as a deliberate release step -- has not been decided; until it is, assume `prod` must be updated by hand before a deploy is expected to include recent work.
21. **Git's default branch renamed `main` -> `dev` (2026-09-14):** Kayela's own request, done locally and pushed (`git branch -m main dev`, `git push -u origin dev`); `origin/main` still exists on GitHub (not deleted -- see the verify table) and GitHub's repository default-branch setting has not been changed (the agent has no `gh` CLI or API access; that is a manual step in GitHub's UI). This git `dev` branch is unrelated to the Neon *database* branch also called `dev` (item 2 above) -- same name, different systems, coincidental collision worth double-checking against in any future instruction that just says "dev."
...
| How `dev` and `prod` stay in sync | Phase 5 | Open -- decide a workflow (auto-sync vs. manual promotion) |
| GitHub default branch still set to `main`; `origin/main` not deleted | Phase 5 | Outstanding -- change the default branch in GitHub's UI (Settings > Branches) before deleting `origin/main`, or new clones and PRs will still target the old branch |
