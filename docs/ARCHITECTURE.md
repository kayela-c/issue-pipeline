...
Model ids stay env-configurable so they can be rolled without a deploy.
...
- The sync endpoint calls `fetch` against `${process.env.URL}/internal/...` with header `x-internal-secret: INTERNAL_JOB_SECRET`, the user's forwarded `Authorization` header, and a JSON body containing the id. It awaits only the 202.
...
- **Token lifetime:** the forwarded Gitea access token must stay valid for the job. The desktop app refreshes its token before calling any job-triggering endpoint if it expires within 20 minutes.
...
3. **Snapshot (cached).** If `repo_snapshots(repo_id, sha)` exists, reuse it. Otherwise:
   - `getTree(sha)` recursively, paging until complete.
   ...
   - Fetch issue templates, supporting both Markdown templates (front matter + body) and YAML issue forms. For YAML forms, convert fields into a Markdown skeleton of `### <field label>` sections.
   - Fetch labels.
...
6. **Link dependencies:** for each dependency, `addDependency(issue, depends_on_number)` -> set `linked_in_gitea = true`. A failure here does not undo the post; record a `link_failed` event (reconcile retries it).
...
1. `listIssuesCreatedBySince(claimed_by.username, claimed_at - 1 min)` and search bodies for the draft's marker.
...
6. Request the minimum scopes needed: read user, read organization, read repository, write issue.
...
## 13. Open decisions and VERIFY items

Resolved at kickoff (2026-09-10):

1. **Gitea reachability** — public HTTPS, reachable from Netlify. Confirmed by Kayela.
2. **Netlify** — account exists; Background Functions availability still to be confirmed on the current plan.
3. **Neon** — dedicated project `issue-pipeline` (`mute-shadow-81137667`), `dev` branch for development.

Still to verify before the phase that depends on them:

| Item | Needed by | Status |
|---|---|---|
| Gitea version + base URL | Phase 1 | **Outstanding — blocks Phase 1** |
| Supported OAuth scopes | Phase 1 | Outstanding |
| Access-token lifetime (`[oauth2] ACCESS_TOKEN_EXPIRATION_TIME`, believed 1 h) | Phase 1 | Outstanding |
| Background Functions on the current Netlify plan | Phase 2 | Outstanding |
| Issue templates endpoint (`GET /repos/{owner}/{repo}/issue_templates`) | Phase 2 | Outstanding |
| Recursive tree pagination and the `truncated` flag | Phase 2 | Outstanding |
| Issue dependencies API, and dependencies enabled per repo | Phase 4 | Outstanding |
| `created_by` / `since` on list-issues | Phase 4 | Outstanding |
| Posting identity: issues authored by the posting user | Phase 4 | Default taken |
| Approval policy: a user may approve their own drafts, recorded in `approved_by` | Phase 3 | Default taken |
| macOS build in v1 | Phase 5 | Outstanding |

### Environment prerequisite (found during Phase 0)

Building the Tauri shell on Windows requires the MSVC toolchain. Visual Studio 18 Community is installed on the dev machine but **without** the "Desktop development with C++" workload, so `link.exe` and the Windows SDK are missing and `cargo build` cannot link. Install that workload from the Visual Studio Installer before running `pnpm dev:desktop`.
